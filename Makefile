...
docker-test-localnet: docker-run-localnet
	docker run -d --name localnet_flow_api_service --rm -p 127.0.0.1:9500:9000 --network localnet_default \
		...
		--protocol-node-addresses=access:9000 --execution-node-addresses=access:9000 --protocol-node-public-keys=aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa --execution-node-public-keys=aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa --rpc-addr=:9000
	sleep 10
