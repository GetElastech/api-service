...
func (fsb *FlowAPIServiceBuilder) Initialize() error {
	...
	fsb.ServiceConfig.Logger.Info().
		Str("upstream-node-addresses", fmt.Sprintf("%v", fsb.ProtocolNodeAddresses)).
		Str("upstream-node-public-keys", fmt.Sprintf("%v", fsb.ProtocolNodePublicKeys))
	fsb.ServiceConfig.Logger.Info().
		Str("upstream-node-addresses", fmt.Sprintf("%v", fsb.ExecutionNodeAddresses)).
		Str("upstream-node-public-keys", fmt.Sprintf("%v", fsb.ExecutionNodePublicKeys))
	return nil
}
