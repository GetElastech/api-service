...
RUN go build -v -tags=relic -o /app main/api_service.go
RUN cp /app/api_service /app/application
...
CMD ["go", "run", "-tags=relic", "main/api_service.go"]
...
RUN rm -rf /go
RUN rm -rf /app
COPY --from=production /app/api_service /bin/api_service
